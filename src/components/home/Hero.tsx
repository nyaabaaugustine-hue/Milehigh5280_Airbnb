  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0 z-10" style={{ background: 'linear-gradient(to bottom, rgba(8,8,8,0.25) 0%, rgba(8,8,8,0.5) 50%, rgba(8,8,8,0.95) 100%)' }} />
        <div className="absolute inset-0 z-10" style={{ background: 'linear-gradient(to right, rgba(8,8,8,0.65) 0%, transparent 60%)' }} />
      </div>
      ...
      <div className="relative z-20 max-w-[1440px] mx-auto px-6 lg:px-12 pb-24 lg:pb-32">
        <div
          className={`inline-flex items-center gap-3 mb-8 transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
          style={{ transitionDelay: '200ms' }}
        >
          <div className="flex gap-1">
            {[0,1,2].map(i => <div key={i} className="w-1.5 h-1.5 rounded-full bg-[var(--gold)] opacity-80" />)}
          </div>
          <span className="section-label">Premium Private Apartment · Ayi Mensah, Accra</span>
        </div>

        <div className="overflow-hidden mb-4">
          <div
            className={`transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-16'}`}
            style={{ transitionDelay: '400ms' }}
          >
            <h1 className="font-serif font-light leading-[0.95] text-white">
        ...
        <p
          className={`text-[var(--text-muted)] font-sans text-base lg:text-lg max-w-md leading-relaxed mb-10 transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
          style={{ transitionDelay: '700ms' }}
        >
          A beautifully furnished private apartment nestled in the lush tranquility of
          Ayi Mensah — where comfort, elegance, and nature converge.
        </p>
        ...
        >
          <button onClick={onBookNow} className="btn-gold">Book Your Stay</button>
