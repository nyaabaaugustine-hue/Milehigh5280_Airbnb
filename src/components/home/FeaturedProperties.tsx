...
function PropertyCard({ property, index, priority, onBookNow, onViewProperty }: PropertyCardProps) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'property-card group transition-all duration-700',
        property.isLive ? 'cursor-pointer' : 'cursor-not-allowed opacity-90 grayscale-[0.8]',
        vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10',
      )}
      style={{ transitionDelay: `${index * 150}ms` }}
      onClick={() => {
        if (!property.isLive) {
          alert(`${property.name} is coming soon! We are currently finalizing the interiors.`);
          return;
        }
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-5 text-xs text-[var(--text-muted)] border-t border-[var(--border)] pt-4 mb-4">
          ...
          <Link
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (!property.isLive) e.preventDefault();
            }}
        ...
        <button
          onClick={(e) => { 
            e.stopPropagation(); 
            if (property.isLive) onBookNow?.(); 
          }}
          className={cn("btn-gold w-full justify-center gap-2 text-[0.7rem] py-3", !property.isLive && "grayscale opacity-50")}
        >
          <Phone size={12} />
          {property.isLive ? 'Book Now' : 'Coming Soon'}
        </button>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {properties.map((property, i) => (
          <PropertyCard
            ...
            property={property}
            index={i}
            priority={i === 0} // Preload the first property card
            onBookNow={onBookNow}
