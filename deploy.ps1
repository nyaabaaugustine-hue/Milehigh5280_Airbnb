...
# Step 1.5: Clean build artifacts to prevent module not found errors
if (Test-Path ".next") {
    Remove-Item ".next" -Recurse -Force
    Write-Host "Cleaned .next build directory" -ForegroundColor Yellow
    Start-Sleep -Seconds 1
}

# Step 2: Create missing page directories
$dirs = @("src/app/privacy", "src/app/terms", "src/app/cancellation", "src/app/properties/[slug]")
foreach ($d in $dirs) {
    if (-not (Test-Path $d)) {
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    git commit -m "chore: local updates and database schema alignment"
}

Write-Host "Integrating fix from other device (Rebasing)..." -ForegroundColor Cyan
git rebase origin/main

if ($LASTEXITCODE -ne 0) {
    Write-Host "Rebase conflict detected! Please resolve manually in VS Code." -ForegroundColor Red
    exit 1
}
